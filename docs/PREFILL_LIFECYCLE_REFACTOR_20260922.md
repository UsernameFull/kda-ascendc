...
### 4.9 assemble 的小传输（本轮新增第六条）：已落地——同样字节、更少调用，生产里位一致

§4.8 把这条线的下一步写成"同样字节、更少调用"。本轮先探针后生产，两步都做了。

**探针**（`kernels/v1/k1_solve_assemble_coalesce_probe.cpp`，12288 chunk，字节固定 100.7 MB，
同进程交错 MIN of 5）：

| 装载形态 | 调用/chunk | ms | 结构（含 L0 链 + store） |
|---|---:|---:|---|
| shipped（2 KB A + 两条 1 KB B band / pass） | 6.0 | 0.313 | 队列 0.706 / 显式 buffer 0.901 |
| 两条 B band 合并（ndNum=KF） | 4.0 | 0.252 | — |
| A 按块合并（ndNum=nch）＋ B 合并 | 2.5 | 0.217 | 队列 0.606 / **显式 buffer 0.531** |
| 全部合并（需重排布局） | 1.0 | 0.220 | — |
| 整块一次 16 KB（上限，需重排布局） | 0.5 | 0.157 | — |

- 2.5 次/chunk 之后**没有再便宜可捡**（1.0 次不更快），所以需要重排工作区的那两支不做。
- 小调用多时队列的 per-chunk 流水值 0.196 ms；换成大调用后**显式 B1 buffer 反超队列 0.075 ms**。
  赢的组合是"合并调用 + 显式 buffer"：0.531 vs shipped 0.706（−25%）。
- 三组位一致检查（P 与 A16）全 IDENTICAL，先于计时。

**生产**（`kda_solve_assemble` 的 `loadMode` 参数，`KDA_ASM_LOADS`，默认 1；两套 L1 形态共用同一个
per-chunk 链函数，公式/dtype/同步结构未动）：

| 口径 | shipped | 合并 | Δ |
|---|---:|---:|---:|
| 输出 | — | 0/1.007e8 个不同，max\|d\| 0 | 位一致 |
| e2e | 10.627 ms | **10.449** | **−0.178** |
| ASM（24-launch 重放） | 0.846 | **0.657** | −0.189 |
| AIC（ASM+cube） | 2.619 | **2.310** | −0.309 |
| sliced（生产 schedule 重放，= stage） | 2.541 | **2.363** | −0.178 |
...
§4.9 把剩下的入口写成 P 往还 0.137 与 cube A16 重读 0.070。本轮两条都做了，都在生产里量：

**cube A16 常驻**（§11.35 的候选，`a16Mode` / `KDA_CUBE_A16_RESIDENT`，默认 1）：

| 口径 | 每 pass 读 | 常驻 L1 | Δ |
|---|---:|---:|---:|
| CUBE 隔离 | 1.577 ms | 1.511 | −0.066 |
| sliced（stage） | 2.346 | 2.327 | −0.019 |
| e2e | 10.445 | 10.421 | −0.023 |

位一致。第一次实现挂核（1 号模式仍对队列 `AllocTensor` 而不 `EnQue` ⇒ 队列泄漏，第二个 pass 卡死），
这类失败和 `KDA_ASM_NCHUNK=6/8` 同源。

**P 上片**（`loadMode` 0/1/2，默认 2）：pass 0 的 fixpipe 用 `CFG_NZ` 把 P 写进 L1，pass 1 用
`LoadDataWithTranspose` 从那里填 L0B，GM 的 P tile 既不写也不读（151.0 → 100.7 MB/call）。

| 口径 | shipped | 合并（P 在 GM） | P 上片 |
|---|---:|---:|---:|
| ASM 隔离 | 0.866 | 0.665 | **0.513** |
| AIC | 2.552 | 2.242 | **2.141** |
| sliced（= stage） | 2.505 | 2.323 | **2.322** |
| e2e | 10.593 | 10.418 | **10.408** |

- 探针口径：上限（完全不要 P）0.107 ms，候选拿走 **0.096（90%）**，且与 shipped 位一致（A16
  0/50331648 个不同，P 的 GM tile 原样）。布局是关键：`CFG_NZ` 的分形顺序是 `[n-block][k-block]`，
  与 Nd2Nz 的 `[k-block][n-block]` 互为转置，所以四个 16×16 分形按 0、2、1、3 进 L0B——**位一致是
  这套映射唯一的验收依据**。
