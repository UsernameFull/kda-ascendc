...
extern "C" __global__ __aicore__ void k2_mix_step(GM_ADDR pw, GM_ADDR ph, GM_ADDR pu,
    ...
    DataCopy(tD1, gD1, E);
    DataCopy(tG, gG, E);
    DataCopy(tT2, gGl, 128);   // g_last in tT2[0:128]
    DataCopy(tUb, gQ, E);      // q bf16
    SetFlag<HardEvent::MTE2_V>(ev2v);
    WaitFlag<HardEvent::MTE2_V>(ev2v);
    Cast(tQ, tUb, RoundMode::CAST_NONE, E);
    AscendC::PipeBarrier<PIPE_V>();
    DataCopy(tUb, gK, E);      // k bf16
    SetFlag<HardEvent::MTE2_V>(ev2v);
    WaitFlag<HardEvent::MTE2_V>(ev2v);
    Cast(tK, tUb, RoundMode::CAST_NONE, E);
    AscendC::PipeBarrier<PIPE_V>();
    DataCopy(tUb, gU, E);      // u bf16
    SetFlag<HardEvent::MTE2_V>(ev2v);
    WaitFlag<HardEvent::MTE2_V>(ev2v);
    ...
    // qg = q * exp2(g)  (non-inplace exp: tU scratch)
    Muls(tU, tG, LN2, E);
    AscendC::PipeBarrier<PIPE_V>();
    Exp(tT, tU, E);
    AscendC::PipeBarrier<PIPE_V>();
    Mul(tU, tQ, tT, E);
    AscendC::PipeBarrier<PIPE_V>();
    Cast(tVb, tU, RoundMode::CAST_RINT, E);
    AscendC::PipeBarrier<PIPE_V>();
    SetFlag<HardEvent::V_MTE3>(evv3);
    WaitFlag<HardEvent::V_MTE3>(evv3);
    DataCopy(gQqg, tVb, E);
    // kg = k * exp2(g_last - g); g_last broadcast from tGl[128]
    for (int r = 0; r < 16; r++) {
        Sub(tT2[r * 128], tT2, tG[r * 128], 128);
        AscendC::PipeBarrier<PIPE_V>();
    }
    Muls(tD1, tT2, LN2, E);
    AscendC::PipeBarrier<PIPE_V>();
    ...
    Mul(tD1, tK, tT2, E);
    AscendC::PipeBarrier<PIPE_V>();
    Cast(tVb, tD1, RoundMode::CAST_RINT, E);
    AscendC::PipeBarrier<PIPE_V>();
    SetFlag<HardEvent::V_MTE3>(evv3);
    WaitFlag<HardEvent::V_MTE3>(evv3);
    DataCopy(gKg, tVb, E);
    ...
    // out = scale*d2 + d3 (reuse tU/tD1/tV as scratch)
    ...
    DataCopy(tU, gD2, E);       // tU = d2
    DataCopy(tD1, gD3, E);      // tD1 = d3
    SetFlag<HardEvent::MTE2_V>(ev2v);
    WaitFlag<HardEvent::MTE2_V>(ev2v);
    Muls(tV, tU, SCALE, E);     // tV = scale*d2
    AscendC::PipeBarrier<PIPE_V>();
    Add(tV, tV, tD1, E);        // tV = scale*d2 + d3
    AscendC::PipeBarrier<PIPE_V>();
    Cast(tVb, tV, RoundMode::CAST_RINT, E);
    AscendC::PipeBarrier<PIPE_V>();
    SetFlag<HardEvent::V_MTE3>(evv3);
    WaitFlag<HardEvent::V_MTE3>(evv3);
    DataCopy(gOut, tVb, E);
    // h_new = h*exp2(g_last) + d4, block loop (8 blocks of 16 rows)
    // reuse tQ (h block), tK (d4 block), tT (coef), tT2 (scratch)
    Muls(tT2, tT2, LN2, 128);
    AscendC::PipeBarrier<PIPE_V>();
    Exp(tT, tT2, 128);
    AscendC::PipeBarrier<PIPE_V>();
    for (int b = 0; b < 8; b++) {
        DataCopy(tQ[b * 2048], gH[b * 2048], DataCopyParams(2, 128, 0, 0));
        DataCopy(tK[b * 2048], gD4[b * 2048], DataCopyParams(2, 128, 0, 0));
        SetFlag<HardEvent::MTE2_V>(ev2v);
        WaitFlag<HardEvent::MTE2_V>(ev2v);
        for (int r = 0; r < 16; r++) {
            Mul(tQ[b * 2048 + r * 128], tQ[b * 2048 + r * 128], tT, 128);
            AscendC::PipeBarrier<PIPE_V>();
        }
        Add(tQ[b * 2048], tQ[b * 2048], tK[b * 2048], 2048);
        AscendC::PipeBarrier<PIPE_V>();
        SetFlag<HardEvent::V_MTE3>(evv3);
        WaitFlag<HardEvent::V_MTE3>(evv3);
        DataCopy(gHnew[b * 2048], tQ[b * 2048], DataCopyParams(2, 128, 0, 0));
    }
#endif
}
