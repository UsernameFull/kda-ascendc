...
// d4 is addressed per (task, chunk) here (d4_reuse == 0 layout).
#include "kernel_operator.h"
using namespace AscendC;
constexpr int32_t M = 16, D = 128, BV = 64, TILE = M * BV;

extern "C" __global__ __aicore__ void kda_k2_outstate_kernel(
    ...
    const uint64_t d4base = static_cast<uint64_t>(c) * D * D + static_cast<uint64_t>(iv) * BV * D;
    ...
    for (int32_t i = 0; i < M; ++i) {
        for (int32_t v = 0; v < BV; ++v) {
            of.SetValue(i * BV + v,
                        scale * d2.GetValue(i * BV + v) + d3.GetValue(i * BV + v));
        }
    }
    ...
    for (int32_t v = 0; v < BV; ++v) {
        for (int32_t k = 0; k < D; ++k) {
            s.SetValue(v * D + k,
                       s.GetValue(v * D + k) * dec.GetValue(k) + d4.GetValue(v * D + k));
        }
    }
