...
extern "C" __global__ __aicore__ void kda_k2_persistent_scan_kernel(
    ...
    TBuf<TPosition::VECCALC> us,uw,uq,uu,uk,ua,udec,d1b,d2b,vb,d3b,d4b,ob,vfb,ofb,wfb,qfb,ufb,kfb,afb;
    pipe.InitBuffer(us,BV*D*4); pipe.InitBuffer(uw,M*D*2); pipe.InitBuffer(uq,M*D*2); pipe.InitBuffer(uu,M*D*2); pipe.InitBuffer(uk,M*D*2); pipe.InitBuffer(ua,M*M*2); pipe.InitBuffer(udec,D*4); pipe.InitBuffer(d1b,TILE*4); pipe.InitBuffer(d2b,TILE*4); pipe.InitBuffer(vb,TILE*2); pipe.InitBuffer(d3b,TILE*4); pipe.InitBuffer(d4b,BV*D*4); pipe.InitBuffer(ob,TILE*2); pipe.InitBuffer(vfb,TILE*4); pipe.InitBuffer(ofb,TILE*4); pipe.InitBuffer(wfb,M*D*4); pipe.InitBuffer(qfb,M*D*4); pipe.InitBuffer(ufb,M*D*4); pipe.InitBuffer(kfb,M*D*4); pipe.InitBuffer(afb,M*M*4);
    LocalTensor<float> s=us.Get<float>(), dec=udec.Get<float>(), d1=d1b.Get<float>(), d2=d2b.Get<float>(), d3=d3b.Get<float>(), d4=d4b.Get<float>(), vf=vfb.Get<float>(), of=ofb.Get<float>(), wf=wfb.Get<float>(), qf=qfb.Get<float>(), uf32=ufb.Get<float>(), kf=kfb.Get<float>(), af=afb.Get<float>();
    LocalTensor<bfloat16_t> wb=uw.Get<bfloat16_t>(), qb=uq.Get<bfloat16_t>(), ub=uu.Get<bfloat16_t>(), kb=uk.Get<bfloat16_t>(), ab=ua.Get<bfloat16_t>(), vbv=vb.Get<bfloat16_t>(), outb=ob.Get<bfloat16_t>();
    ...
    for(int chunk=0;chunk<NT;chunk++) {
        ...
        for(int i=0;i<M;i++) for(int v=0;v<BV;v++){ float x=0,y=0; for(int k=0;k<D;k++){float st=s.GetValue(v*D+k); x+=wf.GetValue(i*D+k)*st; y+=qf.GetValue(i*D+k)*st;} d1.SetValue(i*BV+v,x); d2.SetValue(i*BV+v,y); }
        for(int i=0;i<M;i++) Sub(vf[i*BV],uf32[i*D+iv*BV],d1[i*BV],BV);
